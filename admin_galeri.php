...
if (isset($_GET['delete'])) {
    ...
    header("Location: admin_galeri.php?success=deleted"); exit;
}
...
$search     = trim($_GET['q'] ?? '');
$conditions = [];
if ($filter_kat) $conditions[] = "kategori='" . mysqli_real_escape_string($koneksi,$filter_kat) . "'";
if ($search)     $conditions[] = "judul LIKE '%" . mysqli_real_escape_string($koneksi,$search) . "%'";
...
<style>
.gallery-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(220px,1fr)); gap:1rem; padding:1.5rem; }
.gallery-item {
    border-radius:14px;
    overflow:hidden;
...
.gallery-item:hover { transform:translateY(-4px); box-shadow:0 12px 32px rgba(0,0,0,.12); border-color:#fcd34d; }
.gallery-img { width:100%; aspect-ratio:4/3; object-fit:cover; display:block; }
.gallery-info { padding:.85rem 1rem; }
.gallery-title { font-size:.83rem; font-weight:600; color:#0f172a; margin-bottom:.3rem; line-height:1.4; }
.gallery-meta  { font-size:.72rem; color:#94a3b8; display:flex; align-items:center; gap:.3rem; }
.gallery-actions {
...
.gallery-actions .btn-tbl { width:28px;height:28px;border-radius:7px; }
...
.form-panel.editing { border-color:#fcd34d; box-shadow:0 4px 20px rgba(245,158,11,.12); }
...
.cat-kegiatan  { background:#dbeafe;color:#1d4ed8; }
.cat-prestasi  { background:#dcfce7;color:#15803d; }
.cat-wisuda    { background:#f3e8ff;color:#7c3aed; }
.cat-lainnya   { background:#f1f5f9;color:#64748b; }
</style>
</head>
<body>
...
<div id="admin-content">
    ...
    <div class="admin-page">
        ...
        <?php endif; ?>
        <?php if ($success_msg): ?>
        <div class="alert-admin alert-success mb-4"><i data-lucide="check-circle"></i> <?php echo $success_msg; ?></div>
        <?php endif; ?>
        <?php if ($error_msg): ?>
        <div class="alert-admin alert-error mb-4"><i data-lucide="alert-circle"></i> <?php echo htmlspecialchars($error_msg); ?></div>
        <?php endif; ?>

        <div class="row g-4">
            <!-- LEFT: Form -->
            <div class="col-lg-4">
                <div class="form-panel <?php echo $edit_data ? 'editing' : ''; ?>" style="position:sticky;top:80px;">
                    <h5 style="font-size:.95rem;font-weight:700;color:#0f172a;margin-bottom:1.25rem;display:flex;align-items:center;gap:.5rem;">
                        <i data-lucide="<?php echo $edit_data ? 'pencil' : 'image-plus'; ?>" style="width:18px;height:18px;color:#d97706;"></i>
                        <?php echo $edit_data ? 'Edit Foto' : 'Tambah Foto Baru'; ?>
                    </h5>
                    <?php if ($edit_data): ?>
                    <div style="margin-bottom:1rem;border-radius:12px;overflow:hidden;border:1.5px solid #fcd34d;">
                        <img src="<?php echo htmlspecialchars($edit_data['foto']); ?>" style="width:100%;aspect-ratio:16/9;object-fit:cover;" alt="">
                    </div>
                    <?php endif; ?>
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="file" name="foto" class="form-control-admin" accept="image/*" style="padding:.5rem .75rem;">
                            <div style="font-size:.73rem;color:#94a3b8;margin-top:.4rem;">Format: JPG, PNG, WEBP. Maks. 5MB</div>
                        </div>
                        <div class="mb-4">
            ...
            <div class="col-lg-8">
                ...
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-3">
                    <div class="d-flex gap-2 flex-wrap">
                        ...
                        while($krow=mysqli_fetch_assoc($rk)) $stats_kat[$krow['kategori']]=$krow['cnt'];
                        ...
                        <a href="admin_galeri.php?kat=<?php echo $kv; ?>"
                           ...
                           style="font-size:.78rem;padding:.4rem .85rem;">
                            <?php echo $kl; ?> <span style="opacity:.8;">(<?php echo $stats_kat[$kv] ?? 0; ?>)</span>
                        </a>
                        <?php endforeach; ?>
                    </div>
                ...
                <div class="admin-card">
                    ...
                    <div class="gallery-grid">
                        ...
                        <div class="gallery-item">
                            <img src="<?php echo htmlspecialchars($g['foto']); ?>" class="gallery-img" alt="<?php echo htmlspecialchars($g['judul']); ?>" loading="lazy">
                            <div class="gallery-actions">
                                <a href="admin_galeri.php?edit=<?php echo $g['id']; ?>" class="btn-tbl btn-tbl-edit" title="Edit" style="width:28px;height:28px;">
                                    <i data-lucide="pencil"></i>
                                </a>
                                <a href="admin_galeri.php?delete=<?php echo $g['id']; ?>" class="btn-tbl btn-tbl-delete" title="Hapus"
                                   onclick="return confirm('Yakin hapus foto ini?')" style="width:28px;height:28px;">
                            ...
                            <div class="gallery-info">
                                ...
                                <div class="gallery-meta">
                                    ...
                                    <span style="margin-left:auto;"><?php echo date('d M Y', strtotime($g['created_at'])); ?></span>
                                </div>
                            </div>
                        </div>
                        <?php endwhile; ?>
                    ...
                    <div class="tbl-empty py-5">
                        <div class="tbl-empty-icon"><i data-lucide="image" style="width:28px;height:28px;"></i></div>
                        <?php echo $filter_kat ? 'Tidak ada foto dengan kategori ini.' : 'Belum ada foto di galeri.'; ?>
                        <div style="font-size:.8rem;color:#94a3b8;margin-top:.5rem;">Tambah foto menggunakan form di sebelah kiri</div>
                    </div>
                    <?php endif; ?>
                </div>
            </div>
